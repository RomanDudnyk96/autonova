	...
	function grve_blade_vce_promo_shortcode( $atts, $content ) {
		...
		if ( !empty( $image ) ) {
			...
			if ( !empty( $retina_image ) ) {
				...
				$image_html = wp_get_attachment_image( $img_id, 'full' , "", array( 'class' => $image_class_string, 'srcset'=> $image_srcset ) );
			} else {
				$image_html = wp_get_attachment_image( $img_id, 'full' , "", array( 'class' => $image_class_string ) );
			}
		}
